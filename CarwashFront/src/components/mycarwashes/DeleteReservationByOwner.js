...
export default function DeleteReservationByOwnerF() {
    const classes = useStyles();
    const [bookingIdOwner, setBookingIdOwner] = useState();
    ...
    async function deleteReservationByOwner(credentials) {
        return fetch(`https://localhost:7090/api/booking/cancelbyowner/${parseInt(bookingIdOwner)}`, {
            mode: "cors",
            method: "DELETE",
            headers: {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const response = await deleteReservationByOwner({
                ...
                <div className={classes.paper}>
                    ...
                    <form className={classes.form} noValidate onSubmit={handleSubmit}>
                        <TextField
                            ...
                            name="bookingId"
                            label="bookingId"
                            onChange={(e) => setBookingIdOwner(e.target.value)}
                        />
                        <Button
                            type="submit"
                            fullWidth
